    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb m-0">
                <li class="breadcrumb-item text-dark"><a href="{{ URL::to('admin/features') }}"
                        class="color-changer">{{ trans('labels.features') }}</a></li>
                        ...
                        <div class="row">
                            <div class="form-group">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    <input type="text" class="form-control" name="title" id="title"
                                        value="{{ $editfeature->title }}" placeholder="{{ trans('labels.title') }}"
                                        required>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description" id="description" placeholder="{{ trans('labels.description') }}"
                                    rows="5" required>{{ $editfeature->description }}</textarea>
                            </div>
                            <div class="form-group">
...
@section('scripts')
    <script>
        ...
        function ai_generate() {
            ...
            $.ajax({
                url: "{{ url('admin/features/ai_feature_generate') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    title: title
                ...
                success: function(response) {
                    if (response.success) {
                        $('#description').val(response.feature_description);
