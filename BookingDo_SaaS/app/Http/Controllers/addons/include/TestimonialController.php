...
class TestimonialController extends Controller
{
    public function index(Request $request)
    {
        ...
        $testimonials = Testimonials::where('vendor_id', $vendor_id)->where('user_id', null)->where('service_id', null)->orderBy('reorder_id')->get();
        return view('admin.include.testimonial.index', compact('testimonials'));
    }
    public function add()
    {
        return view('admin.include.testimonial.add');
    }
    ...
    public function edit(Request $request)
    {
        ...
        return view('admin.include.testimonial.edit', compact('edittestimonial'));
    }
    ...
    public function reorder_testimonials(Request $request)
    {
        ...
        $gettestimonials = Testimonials::where('vendor_id', $vendor_id)->get();
        foreach ($gettestimonials as $testimonials) {
            foreach ($request->order as $order) {
                $testimonials = Testimonials::where('id', $order['id'])->first();
                $testimonials->reorder_id = $order['position'];
                $testimonials->save();
            }
        }
        return response()->json(['status' => 1, 'msg' => trans('messages.success')], 200);
