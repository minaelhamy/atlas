    ...
    public function delete(Request $request)
    {
        try {
            ...
            if(Auth::user()->type == 1)
            {
                ...
                $getservice = Service::where(DB::Raw("FIND_IN_SET($checktax->id, replace(tax, '|', ','))"), '>', 0)->get();
                foreach($getservice as $service)
                {
                    $tax = explode('|', $service->tax);
                    $key = array_search($checktax->id, $tax);
                    if ($key !== false) {
                        unset($tax[$key]);
                        Service::where('vendor_id', $vendor_id)->update(array('tax' => implode('|', $tax)));
                    }
                }
            }
            $checktax->delete();
            ...
            dd($th->getMessage());
            return redirect('admin/tax')->with('error', trans('messages.wrong'));
        }
