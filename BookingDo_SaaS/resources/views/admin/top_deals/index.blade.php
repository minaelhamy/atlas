                                ...
                                <form action="{{ URL::to('admin/top_deals/update') }}" method="POST"
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6 form-group">
                                            <label class="form-label">{{ trans('labels.deals_type') }}
                                        ...
                                        <div class="col-sm-6 form-group d-none" id="start_time">
                                            ...
                                            <input type="time" class="form-control" name="top_deals_start_time"
                                                id="top_deals_start_time" value="{{ @$topdeals->start_time }}">
                                        ...
                                        <div class="col-sm-6 form-group d-none" id="end_time">
                                            ...
                                            <input type="time" class="form-control" name="top_deals_end_time"
                                                id="top_deals_end_time" value="{{ @$topdeals->end_time }}">
                                        ...
                                        <div class="col-md-12">
                                            <div
                                                ...
                                                <select class="form-control selectpicker" name="services[]" multiple
                                        ...
                                        <div
                                            ...
                                            <button
                                                class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_top_deals', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
                            ...
                            <tbody>
                                ...
                                @foreach ($servicelist as $key => $service)
                                    <tr class="fs-7 align-middle">
                                         @if (@helper::checkaddons('bulk_delete'))
                                            <td><input type="checkbox" class="row-checkbox form-check-input checkbox-style" value="{{ $service->id }}"></td>
                                        @endif
                                        <td>{{ ++$key }}</td>

                                        <td>{{ $service->name }}</td>
                                        <td>{{ helper::date_formate($service->created_at, $service->vendor_id) }}<br>
                                            {{ helper::time_format($service->created_at, $service->vendor_id) }}
                                        </td>
                                        <td>{{ helper::date_formate($service->updated_at, $service->vendor_id) }}<br>
                                            {{ helper::time_format($service->updated_at, $service->vendor_id) }}
                                        </td>

                                        <td>
                                            <a href="javascript:void(0)" tooltip="{{ trans('labels.delete') }}"
                                                @if (env('Environment') == 'sendbox') onclick="myFunction()" @else onclick="statusupdate('{{ URL::to('admin/top_deals/delete-' . $service->id) }}')" @endif
        ...
        <script>
            $('#deal_type').on('change', function() {
                if ($('#deal_type').val() == 1) {
                    ...
                    $('#top_deals_start_time').prop('required', true);
                    $('#top_deals_end_date').prop('required', true);
                    $('#top_deals_end_time').prop('required', true);
                } else {
