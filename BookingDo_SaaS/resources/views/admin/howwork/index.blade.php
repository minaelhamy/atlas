                            ...
                            <tbody id="tabledetails" data-url="{{ url('admin/how_it_works/reorder_how_work') }}">
                                ...
                                @foreach ($datas as $data)
                                    <tr class="fs-7 row1 align-middle" id="dataid{{ $data->id }}"
                                        ...
                                        <td>{{ helper::date_formate($data->created_at, $vendor_id) }}<br>
                                            {{ helper::time_format($data->created_at, $vendor_id) }}
                                        </td>

                                        <td>{{ helper::date_formate($data->updated_at, $vendor_id) }}<br>
                                        ...
                                        <td>
                                            <div class="d-flex flex-wrap gap-2">
                                                <a href="{{ URL::to('/admin/how_it_works/edit-' . $data->id) }}"
                                                    class="btn btn-info btn-sm hov {{ Auth::user()->type == 4 ? (helper::check_access('role_how_it_works', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
                                                ...
                                                <a href="javascript:void(0)"
                                                    ...
                                                    class="btn btn-danger btn-sm hov {{ Auth::user()->type == 4 ? (helper::check_access('role_how_it_works', Auth::user()->role_id, $vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}"
