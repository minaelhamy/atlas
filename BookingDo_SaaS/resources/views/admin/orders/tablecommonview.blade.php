@php
    ...
    if (request()->is('admin/orders')) {
        $module = 'role_orders';
    } elseif (request()->is('admin/orderreports')) {
        $module = 'role_order_reports';
                ...
                <tr class="fs-7 align-middle">
                    ...
                    <td>
                        <div class="d-flex justify-content-between">
                            <a href="{{ URL::to('/admin/orders/invoice-' . $order->vendor_id . '-' . $order->order_number) }}"
                                class="color-changer">{{ $order->order_number }}</a>
                            @if ($order->vendor_note != '')
                                <a href="javascript:void(0)" class="btn btn-primary btn-sm hov"
                                    tooltip="{{ $order->vendor_note }}">
                                    <i class="fa-solid fa-clipboard"></i>
                                </a>
                            @endif
                        </div>
                    </td>
                    <td>{{ helper::currency_formate($order->grand_total, $order->vendor_id) }}
                        <br>
                    ...
                    @if (@helper::checkaddons('custom_status'))
                        <td>
                            @if ($order->status_type == '1')
                                <span
                                    class="badge bg-warning">{{ @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name == null ? '-' : @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name }}</span>
                            @elseif($order->status_type == '2')
                                <span
                                    class="badge bg-info">{{ @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name == null ? '-' : @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name }}</span>
                            @elseif($order->status_type == '3')
                                <span
                                    class="badge bg-success">{{ @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name == null ? '-' : @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name }}</span>
                            @elseif($order->status_type == '4')
                                <span
                                    class="badge bg-danger">{{ @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name == null ? '-' : @helper::gettype($order->status, $order->status_type, $vendor_id, 2)->name }}</span>
                            @else
                                --
                            @endif
                        </td>
                    @endif
                    <td>{{ helper::date_formate($order->created_at, $vendor_id) }}<br>
                        {{ helper::time_format($order->created_at, $vendor_id) }}
                    </td>
                    <td>{{ helper::date_formate($order->updated_at, $vendor_id) }}<br>
                        {{ helper::time_format($order->updated_at, $vendor_id) }}
                    </td>
                    <td>
                        <div class="d-flex gap-2 flex-wrap align-items-center">
                            <a class="btn btn-sm hov btn-secondary" tooltip="{{ trans('labels.view') }}"
                                href="{{ Auth::user()->type == 4 ? (helper::check_access($module, Auth::user()->role_id, $vendor_id, 'manage') == 1 ? URL::to('/admin/orders/invoice-' . $order->vendor_id . '-' . $order->order_number) : '#') : URL::to('/admin/orders/invoice-' . $order->vendor_id . '-' . $order->order_number) }}">
                                <i class="fa-regular fa-eye"></i>
                            </a>
                            @if (Auth::user()->type == 2 || (Auth::user()->type == 4 && Auth::user()->vendor_id != 1))
                                @if (
                                    ($order->transaction_type == '' || $order->transaction_type == 6 || $order->transaction_type == 1) &&
                                        ($order->payment_status == 1 && $order->status_type != '4' && $order->status_type == '3'))
                                    <a class="btn btn-sm hov btn-primary {{ Auth::user()->type == 4 ? (helper::check_access($module, Auth::user()->role_id, $vendor_id, 'manage') == 1 ? '' : 'd-none') : '' }}"
                                        ...
                                        onclick="payment('{{ $order->order_number }}','{{ $order->grand_total }}','{{ $order->transaction_type }}','{{ helper::image_path($order->screenshot) }}')">
                                        <i class="fa-solid fa-dollar-sign"></i>
                                    </a>
                                @endif
                            @endif
                            <a href="{{ URL::to('admin/orders/print/' . $order->order_number) }}"
                                class="btn btn-dark cursor-pointer hov" tooltip="{{ trans('labels.print') }}">
                                <i class="fa-regular fa-print"></i>
                            </a>
                            <a href="{{ URL::to('/admin/orders/generatepdf/' . $order->vendor_id . '/' . $order->order_number) }}"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header justify-content-between">
                <h5 class="modal-title text-dark color-changer" id="paymentModalLabel">
