...
@section('content')
            <div class="d-flex justify-content-between align-items-center">
                ...
                <h5 class="text-capitalize fw-600 fs-4 color-changer">{{ trans('labels.subscribers') }}</h5>
                                       ...
                                       @php $i=1; @endphp
                                        ...
                                        <tr class="fs-7 align-middle">
                                          ...
                                          <td>@php echo $i++ @endphp</td>
                                            ...
                                            <td>{{ helper::date_formate($subscriber->created_at,$vendor_id) }}<br>
                                                {{ helper::time_format($subscriber->created_at, $vendor_id) }}
                                            </td>
                                            <td>{{ helper::date_formate($subscriber->updated_at,$vendor_id) }}<br>
                                                {{ helper::time_format($subscriber->updated_at, $vendor_id) }}
                                            </td>
                                            <td>

                                                <a tooltip="{{trans('labels.delete')}}" @if (env('Environment') == 'sendbox') onclick="myFunction()" @else  onclick="statusupdate('{{URL::to('admin/subscribers/delete-'.$subscriber->id)}}')" @endif class="btn btn-danger btn-sm hov {{Auth::user()->type == 4 ? (helper::check_access('role_subscribers',Auth::user()->role_id,Auth::user()->vendor_id,'delete') == 1 ? '' : 'd-none') : '' }}"> <i class="fa-regular fa-trash"></i></a>
