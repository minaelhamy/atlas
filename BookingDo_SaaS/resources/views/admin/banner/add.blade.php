                    ...
                    <form action="{{ URL::to('admin/bannersection-1/save-' . $section) }} " method="POST"
                            ...
                            <div class="col-sm-6 form-group 2 selecttype">
                                ...
                                <select class="form-select" name="service" id="service" required>
                                    ...
                                    @foreach ($service as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('service') == $item->id ? 'selected' : '' }}> {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('service')
                        ...
                        <div class="form-group text-{{ session()->get('direction') == '2' ? 'start' : 'end' }} m-0">
                            ...
                            <button
                                ...
                                class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_banners', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">{{ trans('labels.save') }}</button>
                        </div>
                </div>
                </form>
            </div>
