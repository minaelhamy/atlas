                            ...
                            <tbody id="tabledetails" data-url="{{ url('admin/store_categories/reorder_category') }}">
                                ...
                                @foreach ($allcategories as $category)
                                    <tr class="fs-7 row1 align-middle" id="dataid{{ $category->id }}"
                                        ...
                                        <td>
                                            ...
                                            @if ($category->is_available == '1')
                                                <a tooltip="{{ trans('labels.active') }}"
                                                    ...
                                                    class="btn btn-sm btn-outline-success hov {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                            ...
                                            @else
                                                <a tooltip="{{ trans('labels.inactive') }}"
                                                    ...
                                                    class="btn btn-sm btn-outline-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                        ...
                                        <td>
                                            {{ helper::date_formate($category->created_at, $vendor_id) }}<br>
                                            {{ helper::time_format($category->created_at, $vendor_id) }}
                                        </td>

                                        <td>
                                            {{ helper::date_formate($category->updated_at, $vendor_id) }}<br>
                                        ...
                                        <td>
                                            <div class="d-flex flex-wrap gap-2">
                                                <a href="{{ URL::to('admin/store_categories/edit-' . $category->id) }}"
                                                    ...
                                                    class="btn btn-info btn-sm hov {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                ...
                                                <a tooltip="{{ trans('labels.delete') }}"
                                                    ...
                                                    class="btn btn-danger btn-sm hov {{ Auth::user()->type == 4 ? (helper::check_access('role_store_categories', Auth::user()->role_id, Auth::user()->vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}">
