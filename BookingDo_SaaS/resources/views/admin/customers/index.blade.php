                                    ...
                                    <tr class="fs-7 align-middle">
                                            ...
                                            <td>
                                                @if ($user->is_available == 1)
                                                    <a tooltip="{{ trans('labels.active') }}"
                                                        class="btn btn-sm btn-outline-success hov {{ Auth::user()->type == 4 ? (helper::check_access('role_customers', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
                                                ...
                                                @else
                                                    <a tooltip="{{ trans('labels.inactive') }}"
                                                        class="btn btn-sm btn-outline-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_customers', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
                                        ...
                                        <td>
                                            <div class="d-flex flex-wrap gap-2">
                                                @if (Auth::user()->type == 2 || (Auth::user()->type == 4 && Auth::user()->vendor_id != 1))
                                                    <a class="btn btn-sm btn-info hov {{ Auth::user()->type == 4 ? (helper::check_access('role_customers', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
                                                        tooltip="{{ trans('labels.edit') }}"
                                                        href="{{ URL::to('admin/customers/edit-' . $user->id) }}">
                                                        <i class="fa fa-pen-to-square"></i>
                                                    </a>
                                                @endif
                                                <a tooltip="{{ trans('labels.view') }}"
                                                    class="btn btn-sm btn-secondary hov"
                                                    href="{{ URL::to('admin/customers/bookings-' . $user->id) }}">
                                                ...
                                                @if (Auth::user()->type == 2 || (Auth::user()->type == 4 && Auth::user()->vendor_id != 1))
                                                    <a href="javascript:void(0)" tooltip="{{ trans('labels.delete') }}"
                                                        ...
                                                        class="btn btn-danger hov btn-sm {{ Auth::user()->type == 4 ? (helper::check_access('role_customers', Auth::user()->role_id, Auth::user()->vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}">
