    ...
    <tbody>
        ...
        @foreach ($customdomaindata as $ddata)
            <tr class="fs-7 align-middle">
                ...
                <td>
                    <div
                        class="d-flex gap-2 flex-wrap {{ Auth::user()->type == 4 ? (helper::check_access('role_custom_domains', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">
                        @if ($ddata->status == 1)
                            <a tooltip="{{ trans('labels.active') }}" class="btn btn-sm btn-success hov"
                        ...
                        @else
                            <a tooltip="{{ trans('labels.inactive') }}" class="btn btn-sm btn-danger hov"
