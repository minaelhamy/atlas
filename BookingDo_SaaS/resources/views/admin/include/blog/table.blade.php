        ...
        @foreach ($blog as $item)
            <tr class="fs-7 row1 align-middle" id="dataid{{ $item->id }}" data-id="{{ $item->id }}">
                ...
                <td>@php
                    echo $i++;
                @endphp</td>
                <td><img src="{{ helper::image_path($item->image) }}" class="img-fluid rounded hight-50 object-fit-cover"
                        alt=""></td>
                <td>{{ $item->title }}</td>
                ...
                <td>
                    <div class="d-flex gap-2 flex-wrap">
                        <a href="{{ URL::to('admin/blogs/edit-' . $item->slug) }}" tooltip="{{ trans('labels.edit') }}"
                            class="btn btn-info hov btn-sm {{ Auth::user()->type == 4 ? (helper::check_access('role_blogs', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                        ...
                        <a href="javascript:void(0)" tooltip="{{ trans('labels.delete') }}"
                            ...
                            class="btn btn-danger hov btn-sm  {{ Auth::user()->type == 4 ? (helper::check_access('role_blogs', Auth::user()->role_id, Auth::user()->vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}">
