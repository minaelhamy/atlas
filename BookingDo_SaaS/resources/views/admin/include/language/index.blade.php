...
@section('content')
    @if (Auth::user()->type == 1 || (Auth::user()->type == 4 && Auth::user()->vendor_id == 1))
        <div class="alert alert-warning" role="alert">
            <p>{{ trans('labels.dont_use_double_qoute') }}</p>
        </div>
    @endif

    <div class="row settings">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="text-capitalize color-changer fw-600 fs-4">{{ trans('labels.language-settings') }}</h5>
            @if (Auth::user()->type == 1 || (Auth::user()->type == 4 && Auth::user()->vendor_id == 1))
                @if (@helper::checkaddons('language'))
                    <div
                        class="d-inline-flex {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">
                        <a href="{{ URL::to('/admin/language-settings/add') }}" class="btn btn-secondary px-sm-4 d-flex">
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    <div class="dropdown">
                        <div class="d-flex flex-wrap gap-2">
                            <a class="btn btn-info hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"
                            ...
                            @if (Strtolower($currantLang->name) != 'english')
                                <a class="btn btn-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}"
                            ...
                            @if ($currantLang->is_available == '1')
                                @if (helper::available_language('')->count() > 1)
                                    <a tooltip="{{ trans('labels.active') }}"
                                        ...
                                        class="btn btn-success hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                            ...
                            @else
                                <a tooltip="{{ trans('labels.inactive') }}"
                                    ...
                                    class="btn btn-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            ...
                            <select name="language_default" class="form-select" id="language_default"
                                ...
                                @foreach (helper::available_language('') as $item)
                                 ...
                                 @else data-value="{{ URL::to('admin/language-settings/?lang=' . $item->code) }}"> @endif
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                                        ...
                                        <div class="col-lg-12">
                                            <div class="text-{{ session()->get('direction') == '2' ? 'start' : 'end' }}">
                                                <div class="d-flex justify-content-end">
                                                    ...
                                                    @else
                                                        <button type="submit" class="btn btn-raised btn-secondary">
                                                            <i class="fa fa-check-square-o"></i>
                                                            {{ trans('labels.save') }}
                                                        </button>
                                                    @endif
                            ...
                            <table class="table table-striped table-bordered py-3 zero-configuration w-100">
                                ...
                                <tbody>
                                    ...
                                    @foreach (helper::listoflanguage() as $language)
                                        <tr class="fs-7 align-middle" data-id="{{ $language->id }}">
                                            ...
                                            <td>
                                                @if (in_array($language->code, explode('|', helper::appdata($vendor_id)->languages)))
                                                    <a tooltip="{{ trans('labels.active') }}"
                                                        ...
                                                        class="btn btn-sm btn-outline-success hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                        <i class="fas fa-check"></i>
                                                    </a>
                                                @else
                                                    <a tooltip="{{ trans('labels.inactive') }}"
                                                        ...
                                                        class="btn btn-sm btn-outline-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                ...
                                                <div class="d-flex flex-wrap gap-2">
                                                    @if (helper::appdata($vendor_id)->default_language == $language->code)
                                                        <a tooltip="{{ trans('labels.active') }}"
                                                            ...
                                                            class="btn btn-sm btn-success hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                            <i class="fas fa-check"></i>
                                                        </a>
                                                    @else
                                                        <a tooltip="{{ trans('labels.inactive') }}"
                                                            ...
                                                            class="btn btn-sm btn-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_language_settings', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                            <i class="fas fa-close mx-1"></i>
                                                        </a>
                                                    @endif
                                                </div>
