...
@section('content')
    ...
    @php
        ...
        $module = 'role_shipping_management';
    @endphp
                    ...
                    <div class="row">
                        ...
                        <div class="text-{{ session()->get('direction') == '2' ? 'start' : 'end' }}">
                            <button
                                ...
                                class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, $vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">{{ trans('labels.save') }}</button>
                        </div>
                    </div>
                </div>
            </form>
                    ...
                    <div class="card border-0 mb-3 box-shadow">
                        <div class="d-flex justify-content-between align-items-center mx-3 mt-3">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item">
                                        <h5 class="text-capitalize text-dark fs-4 fw-600">
                            ...
                            <a href="{{ URL::to(request()->url() . '/add') }}"
                                class="btn btn-primary px-sm-4 {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}">
                                                ...
                                                <td>
                                                    @if ($content->is_available == '1')
                                                        <a href="javascript:void(0)" tooltip="{{ trans('labels.active') }}"
                                                            ...
                                                            class="btn btn-sm hov btn-outline-success {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                    ...
                                                    @else
                                                        <a href="javascript:void(0)"
                                                            ...
                                                            class="btn btn-sm btn-outline-danger hov {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                            <i class="fa-regular fa-xmark"></i>
                                                        </a>
                                                    @endif
                                                </td>
                                                <td>
                                                    {{ helper::date_formate($content->created_at, $vendor_id) }}<br>
                                                    {{ helper::time_format($content->created_at, $vendor_id) }}
                                                </td>
                                                <td>
                                                    {{ helper::date_formate($content->updated_at, $vendor_id) }}<br>
                                                ...
                                                <td>
                                                    <div class="d-flex flex-wrap gap-2">
                                                        <a href="{{ URL::to('/admin/shipping/edit-' . $content->id) }}"
                                                            ...
                                                            class="btn btn-info btn-sm hov {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, Auth::user()->vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                                        ...
                                                        <a href="javascript:void(0)" tooltip="{{ trans('labels.delete') }}"
                                                            ...
                                                            class="btn btn-danger btn-sm hov {{ Auth::user()->type == 4 ? (helper::check_access('role_shipping_management', Auth::user()->role_id, Auth::user()->vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}">
