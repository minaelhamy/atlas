@if (helper::appdata($vendordata->id)->cart_checkout_countdown == 1)
    <section class="my-2">
        <div class="alert alert-warning fs-7 fw-500 text-center p-2" role="alert">
            ...
            <span id="countdown_after_message" class="text-danger blink_me fw-bold text-capitalize" style="display: none;">
                {{ helper::appdata($vendordata->id)->countdown_expired_message }}
            </span>
...
<script>
    var sec = "{{ helper::appdata($vendordata->id)->countdown_mins * 60 }}",
