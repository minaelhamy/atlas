 ...
 */
function get_default_openai_model()
{
    return get_env_setting('OPENAI_MODEL_DEFAULT', 'gpt-4.1-mini');
}

function get_default_openai_chat_model()
{
...
function normalize_openai_model($model, $type = 'text')
{
    $model = trim((string) $model);

    $legacyMap = [
        'text-ada-001' => 'gpt-4.1-mini',
        'text-babbage-001' => 'gpt-4.1-mini',
        'text-curie-001' => 'gpt-4.1-mini',
    ...
    if ($model === '') {
        return $type === 'image' ? get_default_openai_image_model() : get_default_openai_model();
    }

    if (isset($legacyMap[$model])) {
        return $legacyMap[$model];
    }
...
function get_api_key()
{
    ...
    $envKey = get_env_setting('OPENAI_API_KEY');
    if (!empty($envKey)) {
        return $envKey;
    }
...
function get_image_api_key($type)
{
    ...
    $envKey = get_env_setting('OPENAI_IMAGE_API_KEY', get_env_setting('OPENAI_API_KEY'));
    if ($type === 'openai' && !empty($envKey)) {
        return $envKey;
    }
...
{
    $proxies = get_env_setting('OPENAI_PROXY', get_option('ai_proxies'));
    if (!empty($proxies)) {
