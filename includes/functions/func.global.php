...
/**
 * Read an env setting and cast common boolean strings when needed.
 *
 * @param string $name
 * @param mixed $default
 * @return mixed
 */
function get_env_setting($name, $default = null)
{
    if (!function_exists('atlas_env')) {
        return $default;
    }

    $value = atlas_env($name, $default);
    if (!is_string($value)) {
        return $value;
    }
...
function db_connect()
{
    ...
    $db_port = !empty($config['db']['port']) ? (int) $config['db']['port'] : ini_get("mysqli.default_port");
    $db_connection = new mysqli($config['db']['host'], $config['db']['user'], $config['db']['pass'], $config['db']['name'], $db_port);
...
function load_all_option_in_template(&$config)
{
    ...
    $siteUrl = get_env_setting('SITE_URL');
    if (!empty($siteUrl)) {
        $config['site_url'] = get_site_url($siteUrl);
    }

    $appUrl = get_env_setting('APP_URL');
    if (!empty($appUrl)) {
        $config['app_url'] = get_site_url($appUrl) . "php/";
    }

    $debug = get_env_setting('APP_DEBUG');
    if ($debug !== null) {
        $config['quickad_debug'] = $debug ? 1 : 0;
    }
}
