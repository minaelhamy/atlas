...
foreach ($queryRecords as $row) {
    ...
    if (!empty($row['payment_proof'])) {
        $payment_proof = $config['site_url'] . 'storage/payment_proofs/' . $row['payment_proof'];
        $payment_proof = "<a href='$payment_proof' target='_blank' title='" . __('Payment Proof') . "' data-tippy-placement='top'><i class='fa fa-file-text'></i></a>";
    }
    ...
    $row3 = '<td>' . $premium . '</td>';
    $row4 = '<td>' . $status . '</td>';
    $row5 = '<td>' . $payment_method . ' ' . $payment_proof . '</td>';
